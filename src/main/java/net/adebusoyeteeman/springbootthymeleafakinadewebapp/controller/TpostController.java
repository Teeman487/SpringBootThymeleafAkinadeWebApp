...
import net.adebusoyeteeman.springbootthymeleafakinadewebapp.util.ROLE;
...
public class TpostController { // Spring tea recomend to use interface for injecting the dependency to achieve loose coupling
    ...
    public String tposts(Model model) {
       List<TpostDto> tposts = tpostService.findPostByUser(); //  To allow CRUD operations
        model.addAttribute("tposts", tposts);
    ...
    // create handler method, GET request and return model view [ADMIN PAGE]
    // http://localhost:8080/admin/tposts
    /*@GetMapping("/admin/tposts")
    public String tposts(Model model) {
        List<TpostDto> tposts = tpostService.findAllPosts(); //  To allow CRUD operations
        model.addAttribute("tposts", tposts);
        return "/admin/tposts"; // Thymeleaf view name
...
// 109 Refactor List Posts and Comments Feature for Admin
   ...
   /* @GetMapping("/admin/tposts/comments")
    ...
    // 46  handler method to handle NewPost request
    @GetMapping("admin/tposts/newpost")
    ...
    public String newPostForm(Model model) {
        TpostDto tpostDto = new TpostDto();  // empty to use it in create post thymeleaf template or model will simply convey
    ...
    public String createYourPost(@Valid @ModelAttribute("tpost") TpostDto tpostDto, //Step3: Enable Validation using @Valid Annotation in // handler method to handle form submit request
        ...
        tpostDto.setUrl(getUrl(tpostDto.getProjectTitle()));  // link Url must be set
        tpostService.createYourPost(tpostDto);
        return "redirect:/admin/tposts";  // redirecting to url link at Top
    }
